...
def dywan(x,y,a,b,p): #policzyć wspolrzedne pomniejszych kwadratów do rysowania
    if (p==0):
...
def rectangles(x,y,a,b):
    ...
    glColor3f(1.0, 1.0, 1.0)
    glBegin(GL_TRIANGLES)
    glVertex2f((x+a)/3, (y+b)/3)
    glVertex2f((x+a)/3*2, (y+b)/3)
    glVertex2f((x+a)/3*2, (y+b)/3*2)
    glEnd()

    glColor3f(1.0, 1.0, 1.0)
    glBegin(GL_TRIANGLES)
    glVertex2f((x+a)/3, (y+b)/3)
    glVertex2f((x+a)/3, (y+b)/3*2)
    glVertex2f((x+a)/3*2, (y+b)/3*2)
    glEnd()
